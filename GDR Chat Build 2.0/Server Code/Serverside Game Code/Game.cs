	...
	public class GameCode : Game<Player> {
		...
		public override void UserJoined(Player player)
        {
            ...
            if (!player.PlayerObject.Contains("Type"))
            {
                ...
                {
                    player.UserType = "Mod";
                    player.PlayerObject.Set("Type", "Mod");
                    player.PlayerObject.Save();
                }
                ...
                if (AdminList.IndexOf("|" + player.ConnectUserId.Substring(4) + "|") != -1)
                {
                    ...
                    player.PlayerObject.Set("Type", "Admin");
                    player.PlayerObject.Save();
                }
            ...
            {
                player.UserType = player.PlayerObject.GetString("Type");
            }
            ...
            {
               player.Color = "0xCC0000";
            }
            else if (player.UserType == "Mod")
            {
                player.Color = "0xDC8208"; //"0xD77A41";
            }
            else if (player.UserType == "Dev")
            {
                player.Color = "0x0074FF"; //"0x0098FF";
            }
            else
            {
		...
		public override void GotMessage(Player player, Message message) {
			switch(message.Type) {
				case "ChatMessage": {
                    ...
                    if (m.Length > 600)
                    {
                        player.PlayerObject.Set("Banned", "Large Message");
                        player.PlayerObject.Save();
                        player.Disconnect();
                    }
                    //if (!Regex.IsMatch(m, @"^[a-zA-Z'.]{1,40}$"))
                    //Regex.Replace(m, @"[^\u0000-\u007F]", "");
                    m = Regex.Replace(m, @"[^\u0020-\u007E]", " ");
                    //m = Regex.Replace(m, @"[^&#03-&#300]", " noob ");
                    m = m.Replace("<", "&lt;");
                    m = m.Replace(">", "&gt;");
                    m = m.Replace("&#", "&amp;#");

                    double messageDelayMin = player.UserType == "Mod" ? 400 : player.UserType == "Admin" ? 400 : player.UserType == "Dev" ? 1200 : 1400;
                    if (DateTime.Now.Subtract(player.LastMessage).TotalMilliseconds < messageDelayMin)
                    {
                        return;
                    }
                    //if (!player.CanChat)
                    //{
                        //return;
                    //}
                    int capsMessageLength = player.UserType == "Mod" ? 999 : player.UserType == "Admin" ? 999 : player.UserType == "Dev" ? 30 : 10;
                    if (m.Length > capsMessageLength && m.ToUpper() == m)
                    {
                        m = m.ToLower();
                    }
                    if (m.IndexOf("/system") == 0)
                    {
                    ...
                    if (m.IndexOf("/kick ") == 0)
                    {
                        if (player.ConnectUserId == "kong1138933" || player.UserType == "Admin" || player.UserType == "Mod")//player.UserName == "UnknownGuardian")
                        {
                            Broadcast("ChatMessage", player.Id, m);
                            foreach (Player p in Players)
                            {
                    ...
                    if (m.IndexOf("/id") == 0)
                    {
                        if (player.UserType == "Mod" || player.UserType == "Admin")
                        {
                            foreach (Player p in Players)
                            {
                                if (p.UserName == reciever)
                                {
                                    m = "User ID of " + p.UserName + " is " + p.ConnectUserId;
                                    player.Send("ChatMessage", p.Id, m);
                                    return;
                                }
                            }
                        }
                    ...
                    else if (m.IndexOf("/help") == 0)
                    {
                        player.Status = "Help";
                    }
                    else if (m.IndexOf("/back") == 0 || m.IndexOf("/norm") == 0  || player.Status == "AFK")
                    {
                        player.Status = "Norm";
                    }
                    else if (m.IndexOf("/setColor ") == 0)
                    {
                        if (m.Split(' ').Length > 2 && (player.ConnectUserId == "kong1138933" || player.UserType == "Admin")) //set someone else's color
                        {
                            try
                            {
                                
                                String[] people = m.Split(' ');
                                String colorTo = people[people.Length - 1];
                                if (colorTo == "dev") colorTo = "0x0074FF";
                                if (colorTo == "mod") colorTo = "0xDC8208";
                                if (colorTo == "admin") colorTo = "0xCC0000";
                                people[people.Length - 1] = colorTo;
                                m = String.Join(" ", people);



                                for (int r = 1; r < people.Length - 1; r++)
                                {
                                    foreach (Player p in Players)
                                    {
                                        if (p.UserName == people[r])
                                        {
                                            p.Color = colorTo;
                                        }
                                    }
                            ...
                            {
                                m = "Server Exception: " + e.Message + " Payload: " + m;
                            }
                        }
                        else if(m.Split(' ').Length == 2 && (player.UserType == "Mod" || player.UserType == "Admin"))
                        {
                            String colorTo = m.Split(' ')[1];
                            if (colorTo == "dev") colorTo = "0x0074FF";
                            if (colorTo == "mod") colorTo = "0xDC8208";
                            if (colorTo == "admin") colorTo = "0xCC0000";

                             m = "/setColor " + player.UserName + " " + colorTo;
                             player.Color = colorTo;
                        }
                        else
                        {
                            return;
