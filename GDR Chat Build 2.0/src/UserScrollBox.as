package {
	...
	public class UserScrollBox extends Sprite {
		...
		public function UserScrollBox (s:Sprite) {
			this._chat = s;
		}
		...
		{
			//var type:String = username.substring(username.indexOf("-") + 1);
			trace(username);
			//username = username.substring(0,username.indexOf("-"));
			
			//trace("add users");
			//if its a guest, add the guest number to prevent duplicate users. This should only occur if there is a bug, since no guests are allowed in.
			if(username == "Guest")
			{
				//add a dash and their number
				username += "-" + id;
			}
		...
		private function createRollingScroller(event:Event = null):void
		{
			...
			for(i = 0; i < sortedUsers.length;i++)
			{
				...
				if(sortedUsers[i].UserName == "CircularSq")
				{
					_defaultFormat.color = (Math.random() > 0.5)? 0xFF00CC : 0x00FFFF;
				}
			...
			//Align.centerHorizontallyInStage(_chat, _mask);
			_mask.y = _paddingTop;
			addChild(_mask);
			...
			//swap depths
			//var p:PasteBin = PasteBin(_chat.getChildByName("PasteBin"));
			//p.parent.swapChildren(p,p.parent.getChildAt(p.parent.numChildren-1));
		}
		...
		{
			Log.CustomMetric("Profile Clicked", "User Scroll Box");
